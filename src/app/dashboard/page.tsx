...
export default function Dashboard() {
  ...
  useEffect(() => {
    // If auth is not initialized (e.g. missing API key), we can't protect the route properly
    // but we should fail securely.
    if (!auth) return;
  ...
  // Securely fail if auth was not available
  if (!auth) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background-light dark:bg-background-dark p-4">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-600 mb-2">Service uninitialized</h1>
          <p className="text-stone-600 dark:text-stone-400">Please check your environment configuration.</p>
        </div>
      </div>
    );
  }
