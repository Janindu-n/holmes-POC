...
export default function Dashboard() {
  ...
  useEffect(() => {
    // We must ensure the guard works even if auth is initially null due to env vars
    // In a real app, auth should be configured.
    if (!auth) {
      // If auth is missing, we should probably still not show the dashboard
      // but for this demo environment where we mock env vars, it's tricky.
      // Let's assume if auth is missing, we redirect as well for safety.
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-dashboard-bg dark:bg-background-dark">
        <div className="flex flex-col items-center gap-4">
          <div className="size-12 bg-primary rounded-xl flex items-center justify-center animate-bounce shadow-lg">
            <span className="material-symbols-outlined text-white text-2xl">shield_person</span>
          </div>
          <p className="text-stone-500 dark:text-stone-400 font-medium animate-pulse text-sm">Securing your session...</p>
        </div>
      </div>
    );
  }
