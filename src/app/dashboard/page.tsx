...
export default function Dashboard() {
  ...
  useEffect(() => {
    // Authentication guard to prevent unauthenticated access
    // We check if auth is initialized (not null) to avoid runtime errors during build or if config is missing
    if (!auth) {
    ...
    return (
      <div className="min-h-screen bg-dashboard-bg dark:bg-background-dark flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-stone-500 font-medium animate-pulse">Loading secure dashboard...</p>
        </div>
      </div>
    );
  }
