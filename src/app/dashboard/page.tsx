...
export default function Dashboard() {
  ...
  useEffect(() => {
    // If auth is not available (e.g., missing config), redirect to login
    if (!auth) {
    ...
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (!user) {
        ...
        // Stop loading once we have a user
  ...
  if (authLoading) {
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background-light dark:bg-background-dark font-display">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-stone-800 dark:text-white font-bold animate-pulse">
