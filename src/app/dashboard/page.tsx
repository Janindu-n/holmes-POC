...
export default function Dashboard() {
  ...
  useEffect(() => {
    // If auth is null (service uninitialized), we stay in loading state which is a secure fallback
    if (!auth) return;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-dashboard-bg dark:bg-background-dark">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-stone-600 dark:text-stone-300 font-medium">Verifying access...</p>
        </div>
      </div>
    );
  }
