...
/**
 * HeroParallax component optimizes the hero section scroll animations.
 * By using requestAnimationFrame and direct DOM manipulation via refs,
 * we avoid React re-renders on every scroll tick, significantly improving
 * performance on the landing page.
 */
export default function HeroParallax() {
  ...
  useEffect(() => {
    ...
    // Use a ticking flag to throttle updates to once per repaint cycle
    const updateParallax = () => {
      if (isUnmounted || !containerRef.current) return;

      const scrollY = window.scrollY;

      // Calculate values (logic preserved from original implementation)
      const opacity = Math.max(1 - scrollY / 400, 0);
      const translateY = scrollY / 3;

      // Direct DOM manipulation bypasses React reconciliation for 60fps updates
    ...
    // Synchronize visual state immediately on mount
    updateParallax();
