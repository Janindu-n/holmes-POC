...
/**
 * HeroParallax component isolates high-frequency scroll events
 * to prevent re-rendering the entire landing page.
 * Uses passive scroll listeners and requestAnimationFrame for optimal performance.
 */
export default function HeroParallax() {
  ...
  useEffect(() => {
    ...
    // Performance optimization: use passive listener to avoid blocking main thread
    window.addEventListener('scroll', handleScroll, { passive: true });

    // Initial sync with scroll position
    window.requestAnimationFrame(() => {
      setScrollY(window.scrollY);
    });
  ...
  // Performance win: these calculations only trigger re-renders of this leaf component
